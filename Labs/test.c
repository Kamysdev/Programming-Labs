...
//    sqlite3_reset(stmt);                                        //Сброс запроса     
//    int count = 0;                                              //Счётчик резерва памяти
//    int step = sqlite3_step(stmt);                              //Шаг по запросу                          
//    while (step == SQLITE_ROW) {
//        count++;
//        step = sqlite3_step(stmt);                              //Следующий шаг по запросу
//    }
//
//    sqlite3_bind_text(stmt, 1, lo_hire_date, -1, 0);
//    sqlite3_bind_text(stmt, 2, hi_hire_date, -1, 0);
//
//    (*dest) = (struct Result*) malloc(sizeof(struct Result) * count);            //Выделяем ячейки памяти (объём) для массива
//    sqlite3_reset(stmt); 
//
